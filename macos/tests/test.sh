...
for script in "$macos_root"/runtime/*.zsh "$macos_root"/scripts/*.sh "$macos_root"/scripts/listenote-daily; do
  /bin/zsh -n "$script"
done
...
converted=$(printf '最近兩個月的需求文檔' | "$macos_root/prebuilt/zh-simplify")
[ "$converted" = '最近两个月的需求文档' ]

file "$macos_root/prebuilt/Listenote Daily.app/Contents/MacOS/ListenoteDaily" | grep -q 'universal binary'
file "$macos_root/prebuilt/zh-simplify" | grep -q 'universal binary'
codesign --verify --deep --strict "$macos_root/prebuilt/Listenote Daily.app"
